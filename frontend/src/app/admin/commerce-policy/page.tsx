          ...
          <div className="space-y-6">
            ...
            <section className="rounded-xl border border-emerald-200 bg-emerald-50/50 p-5">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <h2 className="text-lg font-semibold text-slate-900">eBay market price suggestions</h2>
                  <p className="mt-1 text-sm text-slate-600">
                    These settings only calculate suggestions. Prices are never changed until an administrator
                    selects rows and clicks Apply in Admin → eBay Market.
                  </p>
                </div>
                <label className="inline-flex items-center gap-2 rounded-lg border border-emerald-200 bg-white px-3 py-2 text-sm font-medium text-slate-800">
                  <input
                    type="checkbox"
                    checked={form.price_sync_enabled}
                    onChange={(e) => update('price_sync_enabled', e.target.checked)}
                  />
              ...
              <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
                <Field
                  label="Median price factor"
                  hint="Suggested price = eBay median × this factor. Use 1.00 to follow the median directly."
                >
                  <input
                    type="number"
                    min={0.1}
                    ...
                    step={0.01}
                    className={inputClass}
                    value={form.price_sync_factor}
                    onChange={(e) => update('price_sync_factor', numberValue(e.target.value, form.price_sync_factor))}
                  />
                </Field>
                <Field label="Minimum matching listings" hint="No suggestion is offered below this sample count.">
                  <input
                    type="number"
                    min={1}
                    max={100}
                    step={1}
                    className={inputClass}
                    value={form.price_sync_min_samples}
                    onChange={(e) => update('price_sync_min_samples', numberValue(e.target.value, form.price_sync_min_samples))}
                  />
                </Field>
                <Field
                  label="Maximum automatic delta (%)"
                  hint="Larger changes stay visible but are blocked from Apply and require manual investigation."
                >
                  <input
                    type="number"
                    min={1}
                    ...
                    step={1}
                    className={inputClass}
                    value={form.price_sync_max_delta_pct}
                    onChange={(e) => update('price_sync_max_delta_pct', numberValue(e.target.value, form.price_sync_max_delta_pct))}
                  />
                </Field>
                <Field
                  label="Round suggestion to"
                  hint="0 keeps cents; 1 rounds to whole dollars; 5 rounds to the nearest $5."
                >
                  <input
                    type="number"
                    min={0}
                    ...
                    step={0.01}
                    className={inputClass}
                    value={form.price_sync_round_to}
                    onChange={(e) => update('price_sync_round_to', numberValue(e.target.value, form.price_sync_round_to))}
                  />
                </Field>
              </div>
              <div className="mt-4 rounded-lg border border-emerald-200 bg-white p-3 text-sm text-emerald-900">
                Recommended starting values: factor <strong>1.00</strong>, minimum samples <strong>3</strong>,
                maximum delta <strong>50%</strong>, rounding <strong>0</strong>. Review results before changing the factor.
              </div>
            </section>
