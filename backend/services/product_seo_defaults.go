...
func BuildDefaultProductSEO(product *models.Product) EnrichedProduct {
	...
	return EnrichedProduct{
		...
		MetaTitle: BuildSafeMetaTitle(
			fmt.Sprintf("%s %s %s | VIBO CNC", brandLabel, model, partType),
			fmt.Sprintf("%s %s | VIBO CNC", brandLabel, model),
			fmt.Sprintf("%s %s | VIBO CNC", model, partType),
			fmt.Sprintf("%s | VIBO CNC", model),
			fmt.Sprintf("%s | VIBO CNC", strings.TrimSpace(product.Name)),
		),
		MetaDescription: BuildSafeMetaDescription(
			...
			fmt.Sprintf("%s %s available from VIBO CNC with compatibility support and global delivery.", model, partType),
		),
